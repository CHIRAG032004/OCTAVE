                ...
                <div className='flex items-start justify-between gap-3'>
                  <div className='flex-1 min-w-0'>
                    ...
                    <p className='text-sm text-zinc-600 mt-1 line-clamp-3 max-h-12 overflow-hidden'>
                      {issue.userMessage || 'No description provided.'}
                    </p>
                  </div>

                  <div className='flex flex-col items-end gap-2'>
                    <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                      normalizeIssueStatus(issue.status) === 'resolved' ? 'bg-emerald-100 text-emerald-800' :
                      normalizeIssueStatus(issue.status) === 'in progress' ? 'bg-amber-100 text-amber-800' :
