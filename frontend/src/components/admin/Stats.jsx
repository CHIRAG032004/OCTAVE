    ...
    <section className='mt-4'>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-gradient-to-br from-green-300 to-green-400 shadow-lg px-6 py-6 rounded-xl flex flex-col">
            ...
            <span className="text-4xl font-extrabold text-zinc-900">{issues.filter(issue => normalizeIssueStatus(issue.status) === 'resolved').length}</span>
          </div>
          <div className="bg-gradient-to-br from-red-300 to-red-400 shadow-lg px-6 py-6 rounded-xl flex flex-col">
            ...
            <span className="text-4xl font-extrabold text-zinc-900">{issues.filter(issue => normalizeIssueStatus(issue.status) === 'pending').length}</span>
          </div>
          <div className="bg-gradient-to-br from-yellow-200 to-yellow-300 shadow-lg px-6 py-6 rounded-xl flex flex-col">
            ...
            <span className="text-4xl font-extrabold text-zinc-900">{issues.filter(issue => normalizeIssueStatus(issue.status) === 'in progress').length}</span>
          </div>
        </div>
      </section>
  )
